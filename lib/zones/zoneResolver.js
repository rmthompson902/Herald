 ...
 * @param {Map<string,string>} patchMap - patch id (as string) -> zone name
 * @param {string} cueNumber
 * @returns {Promise<{ zones: string[], zoneDetails: Object<string, { cueNumber: string, durationSeconds: (number|undefined), qlabInternalId: (string|undefined) }> }>}
 */
async function resolveZoneDetailsForCue(qlabProtocol, patchMap, cueNumber) {
  const { zones, zoneToCueNumber } = await resolveZoneInfoForCue(qlabProtocol, patchMap, cueNumber);
