 ...
 */
async function collectZoneInfo(qlabProtocol, patchMap, node, zones, zoneToCueNumber, unmappedLeafCues) {
  if (Array.isArray(node.cues) && node.cues.length > 0) {
    ...
    await Promise.all(
      node.cues.map((child) => collectZoneInfo(qlabProtocol, patchMap, child, zones, zoneToCueNumber, unmappedLeafCues))
    );
    return;
  }
 ...
 * @param {Map<string,string>} patchMap - patch id (as string) -> zone name
 * @param {string} cueNumber
 * @returns {Promise<{ zones: string[], zoneToCueNumber: Map<string,string>, unmappedLeafCues: Array<{cueNumber: string, patchId: number}> }>}
 */
async function resolveZoneInfoForCue(qlabProtocol, patchMap, cueNumber) {
  ...
  if (!node) return { zones: [], zoneToCueNumber: new Map(), unmappedLeafCues: [] };
  ...
  return { zones: Array.from(zones), zoneToCueNumber, unmappedLeafCues };
}
 ...
 * @param {Map<string,string>} patchMap - patch id (as string) -> zone name
 * @param {string} cueNumber
 * @returns {Promise<{ zones: string[], zoneDetails: Object<string, { cueNumber: string, durationSeconds: (number|undefined), qlabInternalId: (string|undefined) }>, unmappedLeafCues: Array<{cueNumber: string, patchId: number}> }>}
 */
async function resolveZoneDetailsForCue(qlabProtocol, patchMap, cueNumber) {
  const { zones, zoneToCueNumber, unmappedLeafCues } = await resolveZoneInfoForCue(qlabProtocol, patchMap, cueNumber);
  ...
  return { zones, zoneDetails, unmappedLeafCues };
}
