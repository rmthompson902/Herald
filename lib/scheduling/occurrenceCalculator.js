...
/**
 * All grid-aligned occurrence times for one calendar day (`cursorDay`), stepping every
 * intervalSeconds starting from the day's startTime (or midnight). Values can exceed
 * SECONDS_PER_DAY when the window wraps past midnight (endTime < startTime) - callers
 * turn these into real Date objects via `Date.prototype.setSeconds`, which normalizes
 * overflow into the correct following calendar day on its own.
 *
 * Overnight windows are inferred, not an explicit field: endTime < startTime means "wraps
 * to the next day." Whether the wrap is honored depends on whether the *next* calendar day
 * is itself allowed for this schedule (weekdays + date range, via `isDayAllowed`):
 *   - next day allowed  -> full wrap (e.g. Fri 11pm -> Sat 2am, both days checked)
 *   - next day not allowed -> truncate at 23:59:59 of cursorDay (e.g. only Fri checked)
 * See docs/adr/0013-overnight-schedule-windows.md.
 */
function gridTimesForDay(scheduleRow, cursorDay) {
  const startSeconds = parseTimeToSeconds(scheduleRow.startTime) ?? 0;
  const endSeconds = parseTimeToSeconds(scheduleRow.endTime) ?? SECONDS_PER_DAY - 1;
  const interval = scheduleRow.intervalSeconds;

  let effectiveEnd = endSeconds;
  if (scheduleRow.startTime != null && scheduleRow.endTime != null && endSeconds < startSeconds) {
    const nextDay = new Date(cursorDay);
    nextDay.setDate(nextDay.getDate() + 1);
    effectiveEnd = isDayAllowed(scheduleRow, nextDay)
      ? endSeconds + SECONDS_PER_DAY
      : SECONDS_PER_DAY - 1;
  }

  const times = [];
  for (let t = startSeconds; t <= effectiveEnd; t += interval) {
    times.push(t);
  }
  return times;
}
...
function nextOccurrences(scheduleRow, from, count) {
  ...
  while (results.length < count && daysScanned < MAX_DAYS_TO_SCAN) {
    if (isDayAllowed(scheduleRow, cursorDay)) {
      ...
      for (const seconds of gridTimesForDay(scheduleRow, cursorDay)) {
