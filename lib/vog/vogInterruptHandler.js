...
// that same scope - see docs/03-domain-concepts.md and
// docs/adr/0009-vog-preemption.md. Anything interrupted (including still-queued, not-yet-fired entries) is
...
async function triggerVog(deps, vogMessage) {
  ...
  const { zones, zoneDetails, cueDisplayName } = await resolveZoneDetailsForCue(
    vogMessage.qlabCueNumber
  );
  ...
  if (zones.length > 0) {
    ...
    await Promise.all(
      Array.from(cueNumbersToStop).map((cueNumber) =>
        Promise.resolve(qlabProtocol.stopCue(cueNumber)).catch(() => {})
      )
    );
