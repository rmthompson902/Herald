...
class ZoneQueueEngine {
  ...
  _fire(entry) {
    if (entry.zone) {
      ...
      this._occupancy.set(entry.zone, { entry, expectedEndAt, firedAt, knownDurationMs, timer, confirmed: true });
    }
  ...
  _confirmStopped(qlabInternalId) {
    for (const [zone, occ] of Array.from(this._occupancy.entries())) {
      if (occ.confirmed && occ.entry.qlabInternalId === qlabInternalId) {
        ...
        const suspected = occ.knownDurationMs != null && elapsedMs < occ.knownDurationMs * this._suspectedFailureRatio;
