...
// Per-zone occupancy + FIFO wait queue + stale-drop + VOG preempt, per the
// docs/adr/ records. Pure/in-memory - nothing here is durable by
// design (see docs/adr/0001-per-zone-fifo-queue-model.md: losing in-flight queue state on restart is correct, cron-plus jobs are
// fully rebuilt from schedules on boot instead of resuming any in-flight queue).
//
// The engine is genuinely N independent per-zone FIFOs (see docs/adr/0004-independent-per-zone-fifos.md) - a
...
// A real completion confirmed via the live /updates path (see _confirmStopped) can land a
// little before our own cached duration - fade tails, rounding (see ADR decision 3's own
// note on this same slack). Real-world jitter observed so far tops out around a few hundred
// ms on multi-second cues. A cue confirmed stopped at less than half its own known duration
// is nowhere close to that - found via a real investigation where a cue whose Audio Patch's
// output device had been disconnected was confirmed stopped ~50-80ms after firing, against a
// real 9.292s duration (QLab never denies the /start itself in this case - see
// test/fixtures/qlab-osc-findings.md and docs/adr/0001, decision 2's amendment). 50% gives
// wide headroom above documented normal jitter while still catching that case by ~18x margin.
const DEFAULT_SUSPECTED_FAILURE_RATIO = 0.5;
...
class ZoneQueueEngine {
   ...
   * @param {number} [options.suspectedFailureRatio] - see DEFAULT_SUSPECTED_FAILURE_RATIO
   * @param {(event: string, entry: object, extra?: object) => void} [options.onEvent] - business
   *   event hook (fired/queued/dropped_stale/queue_overflow_dropped/zone_freed/
   *   suspected_playback_failure/confirm_wait/confirm_timeout_firing_anyway/duck_wait/
   *   unduck_wait) - wire to lib/log/eventLogger.js from the caller, not logged internally.
   *   `suspected_playback_failure` fires INSTEAD OF `zone_freed` (never both) when the live
   *   /updates path (see _confirmStopped) confirms a cue stopped implausibly early relative
   *   to its own known duration - see DEFAULT_SUSPECTED_FAILURE_RATIO for why/when. Its
   *   `extra` carries `{ zone, elapsedMs, expectedMs }` instead of just `{ zone }`. Every call
   *   is also recorded in the recent-events ring buffer regardless of whether a hook is
   *   provided - see getRecentEvents().
  ...
  constructor(qlabProtocol, options = {}) {
    ...
    this._fallbackDurationSeconds =
      options.fallbackDurationSeconds ?? DEFAULT_FALLBACK_DURATION_SECONDS;
    ...
    this._suspectedFailureRatio = options.suspectedFailureRatio ?? DEFAULT_SUSPECTED_FAILURE_RATIO;
   ...
   *     durationSeconds, dueAt, name, cueDisplayName, source }
   * `name` is the schedule/VOG message's own configured name; `cueDisplayName` is the cue's
   * actual name in QLab (from cue_cache/refreshCueCache) - kept distinct rather than
   * collapsed into one field so a caller can prefer whichever it wants (the visualizer
   * prefers `cueDisplayName`, same as it already does for projected upcoming occurrences -
   * see zoneUpcomingOccurrences.js).
  ...
  async enqueue(entry) {
    ...
    const settlePromises = subEntries
      .map((sub) => this._settling.get(sub.zone)?.promise)
      .filter(Boolean);
   ...
   * `firedAt`/`knownDurationMs` mirror _fire()'s own occupancy fields so callers (the queue
   * visualizer) can compute a live countdown without re-deriving it - `knownDurationMs: null`
   * means "duration unknown" and must never be treated as (or coerced to) the 30s fallback
   * duration, same rule _fire()/_confirmStopped already follow internally.
   */
  getState() {
    const occupancy = {};
    for (const [zone, occ] of this._occupancy) {
      occupancy[zone] = {
        entry: occ.entry,
        expectedEndAt: occ.expectedEndAt,
        confirmed: occ.confirmed,
        firedAt: occ.firedAt ?? null,
        knownDurationMs: occ.knownDurationMs ?? null
      };
    }
    const queued = {};
  ...
  _compareEntries(a, b) {
    ...
    return (
      String(a.cueNumber).localeCompare(String(b.cueNumber)) ||
  ...
  _fire(entry) {
    if (entry.zone) {
      ...
      const firedAt = this._clock();
      const expectedEndAt = firedAt + durationMs;
      // Only a genuinely known (live-resolved) duration is a meaningful baseline for
      // _confirmStopped's early-completion check below - the 30s fallback is a deliberately
      // generous safety net for an unknown duration, not a real expectation, so comparing
      // against it would misfire on a real short cue whose duration just failed to resolve.
      const knownDurationMs = entry.durationSeconds != null ? entry.durationSeconds * 1000 : null;
      const timer = this._setTimer(() => this._freeZone(entry.zone, entry), durationMs);
      this._occupancy.set(entry.zone, {
        entry,
        expectedEndAt,
        firedAt,
        knownDurationMs,
  ...
  _freeZone(zone, entry, eventName = 'zone_freed', extraFields = {}) {
    const occ = this._occupancy.get(zone);
    if (!occ || occ.entry !== entry) return;
    this._occupancy.delete(zone);
    this._emit(eventName, entry, { zone, ...extraFields });
  ...
  _confirmStopped(qlabInternalId) {
    for (const [zone, occ] of Array.from(this._occupancy.entries())) {
      if (occ.confirmed && occ.entry.qlabInternalId === qlabInternalId) {
        ...
        // A cue confirmed stopped (via a real /updates push + live isRunning re-query - see
        // handleQlabUpdate) well before its own known duration elapsed almost certainly never
        // actually played - found via a real investigation (see DEFAULT_SUSPECTED_FAILURE_RATIO)
        // where a disconnected Audio Patch device caused exactly this: QLab never denies the
        // /start OSC command itself, so this early-stop gap is the only observable signal.
        // Distinct event INSTEAD OF the normal zone_freed (never both), so the operator-facing
        // log and the webapp's toast notification can tell "genuinely played to completion"
        // apart from "silently failed" - both currently free the zone identically otherwise.
        const elapsedMs = Math.max(0, this._clock() - occ.firedAt);
        const suspected =
          occ.knownDurationMs != null &&
          elapsedMs < occ.knownDurationMs * this._suspectedFailureRatio;

        this._freeZone(
          zone,
          occ.entry,
          suspected ? 'suspected_playback_failure' : 'zone_freed',
          suspected ? { elapsedMs, expectedMs: occ.knownDurationMs } : {}
        );
      }
    }
  }
