...
class QlabProtocol {
   ...
   * losing it.
   *
   * Uses requestOverTcp() (TCP+SLIP), not request() (UDP): this reply is the full cue tree,
   * and for a large workspace it can exceed what a single UDP datagram can carry (confirmed
   * live: ~653,000 bytes for a real show file, against UDP's ~65,507-byte practical
   * ceiling) - see docs/adr/0012-cuelists-tcp-transport.md. The in-flight dedup above is
   * unchanged; only the transport underneath it differs.
   */
  async getCueLists() {
    if (!this._pendingCueListsRequest) {
      this._pendingCueListsRequest = this._client.requestOverTcp('/cueLists').finally(() => {
