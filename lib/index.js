...
const zoneUpcomingOccurrences = require('./scheduling/zoneUpcomingOccurrences');
...
function createCore(config) {
  ...
  const healthMonitor = new HealthMonitor(qlabProtocol, oscClient, {
    log: config.appLogger ? config.appLogger('healthMonitor') : undefined
  });
  ...
  const queueEngine = new ZoneQueueEngine(qlabProtocol, {
    onEvent: config.onQueueEvent,
    onZoneTransition
  });
  ...
  if (config.appLogger) {
    ...
    oscClient.on('message', (msg) => {
      ...
      const argsText =
        payload !== undefined ? JSON.stringify(payload) : JSON.stringify(msg.args ?? []);
    ...
    // Routed diagnostic copy of the transport-error, in addition to oscClient's own
    // dependency-free console fallback (see oscClient.js) - this one lands durably in
    // app-*.log rather than only console output, when a logger is actually configured.
    oscClient.on('error', (err) => oscLog.warn(`OSC transport error: ${err.message}`));
  }
  ...
  return {
    ...
    vog: {
      trigger: (vogMessage) =>
        triggerVog(
          { qlabProtocol, resolveZoneDetailsForCue, queueEngine, duckImmediately },
          vogMessage
        )
    }
  };
