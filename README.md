...
audio **zones** of a venue, and trigger one-tap **Voice-of-God (VOG)** emergency messages, all
without hand-building a single playback event in QLab. QLab stays the sole owner of audio; this
system owns the timing, the collision rules, and the operator interface.

It runs entirely on the same Mac as QLab, over loopback OSC — no cloud, no external dependencies.
...
Every zone runs its own independent queue. This page shows, in real time (pushed over SocketIO),
what is playing in each zone, what is waiting behind it, and what is coming up next.

![Zone Queue visualizer](docs/images/zone-queues-visualizer.png)

### Automatic zone derivation
Zones aren't tagged by hand — they're **derived from each cue's QLab Messaging Audio Patch
...
- **QLab** owns all audio — media, cues, routing, fades, ducking.
...
[07 · Deployment & operations](docs/07-deployment-operations.md). For local development —
running the engine and webapp by hand, tests, and linting — see
