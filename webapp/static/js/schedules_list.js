...
function loadNextOccurrences() {
  ...
  ScheduleAPI.getNextOccurrences()
    .then((result) => {
  ...
  document.querySelectorAll('.toggle-schedule-btn').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      if (result) {
        window.showToast(
          'Schedule Updated',
          result.message || window.AppConstants.MESSAGES.SCHEDULE_TOGGLED,
          'success'
        );
  ...
  if (enableAllBtn) {
    enableAllBtn.addEventListener('click', async () => {
      ...
      if (result) {
        window.showToast(
          'Schedules Updated',
          window.AppConstants.MESSAGES.SCHEDULES_ENABLED_ALL,
          'success'
        );
        setTimeout(() => window.location.reload(), 600);
      }
      ...
      ModalManager.showConfirmation(
        ...
        async () => {
          ...
          if (result) {
            window.showToast(
              'Schedules Updated',
              window.AppConstants.MESSAGES.SCHEDULES_DISABLED_ALL,
              'success'
            );
            setTimeout(() => window.location.reload(), 600);
          }
